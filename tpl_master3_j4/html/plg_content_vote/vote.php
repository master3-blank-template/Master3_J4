...
<form method="post" action="<?php echo htmlspecialchars($uri->toString(), ENT_COMPAT, 'UTF-8'); ?>" class="form-inline mb-2">
    <span class="content_vote uk-flex uk-margin-small-top">
        <label class="uk-hidden" for="content_vote_<?php echo (int) $row->id; ?>"><?php echo Text::_('PLG_VOTE_LABEL'); ?></label>
        <?php echo HTMLHelper::_('select.genericlist', $options, 'user_rating', 'class="uk-select uk-form-small uk-width-auto uk-margin-small-right"', 'value', 'text', '5', 'content_vote_' . (int) $row->id); ?>
        <input class="uk-button uk-button-small uk-button-primary" type="submit" name="submit_vote" value="<?php echo Text::_('PLG_VOTE_RATE'); ?>">
