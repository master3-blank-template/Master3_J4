...
if (is_array($button->get('options')) && isset($options['confirmText']) && isset($options['confirmCallback'])) {
    $confirm = '<button type="button" class="btn btn-success" data-bs-dismiss="modal" onclick="' .
        $options['confirmCallback'] . '">' . $options['confirmText'] . ' </button>';
}
...
echo HTMLHelper::_(
    ...
    array(
        ...
        'footer' => $confirm . '<button type="button" class="uk-button uk-button-primary" data-bs-dismiss="modal" ' . $onclick . '>'
