...
$attr .= !empty($class) ? ' class="uk-input form-control field-media-input ' . $class . '"' : ' class="uk-input form-control field-media-input"';
...
$modalHTML = HTMLHelper::_(
    ...
    [
        ...
        'bodyHeight'  => '60',
        'class'       => 'uk-open',
        'footer'      => '<button type="button" class="uk-button uk-button-primary button-save-selected" ' . $onclick . '>' . Text::_('JSELECT') . '</button>'
            . '<button type="button" class="uk-button uk-button-secondary" data-bs-dismiss="modal" ' . $onclick . '>' . Text::_('JCANCEL') . '</button>',
    ]
);
...
?>
<joomla-field-media class="field-media-wrapper" type="image" base-path="<?php echo Uri::root(); ?>" root-folder="<?php echo ComponentHelper::getParams('com_media')->get('file_path', 'images'); ?>" url="<?php echo $url; ?>" modal-container=".modal" modal-width="100%" modal-height="400px" input=".field-media-input" button-select=".button-select" button-clear=".button-clear" button-save-selected=".button-save-selected" preview="static" preview-container=".field-media-preview" preview-width="<?php echo $previewWidth; ?>" preview-height="<?php echo $previewHeight; ?>" supported-extensions="<?php echo str_replace('"', '&quot;', json_encode(['images' => $imagesAllowedExt, 'audios' => $audiosAllowedExt, 'videos' => $videosAllowedExt, 'documents' => $documentsAllowedExt])); ?>">
    <?php echo $modalHTML; ?>
    <?php if ($showPreview) { ?>
        <div class="field-media-preview uk-width">
            <?php echo ' ' . $previewImgEmpty; ?>
            <?php echo ' ' . $previewImg; ?>
        </div>
    <?php } ?>
    <div class="uk-button-group uk-width" style="max-width:356px">
        <input type="text" name="<?php echo $name; ?>" id="<?php echo $id; ?>" value="<?php echo htmlspecialchars($value, ENT_COMPAT, 'UTF-8'); ?>" readonly="readonly" <?php echo $attr; ?>>
        <?php if ($disabled != true) { ?>
            <button type="button" class="uk-button uk-button-primary button-select" data-uk-toggle="target:<?php echo '#imageModal_' . $id; ?>">
                <?php if ($jsIcons) { ?>
                    <span data-uk-icon="icon:image" aria-hidden="true" style="min-width:20px;"></span>
                    <span class="visually-hidden uk-hidden"><?php echo Text::_('JLIB_FORM_BUTTON_SELECT'); ?></span>
                <?php } else { ?>
                    <span><?php echo Text::_('JLIB_FORM_BUTTON_SELECT'); ?></span>
                <?php } ?>
            </button>
            <button type="button" class="uk-button uk-button-danger button-clear">
                <?php if ($jsIcons) { ?>
                    <span data-uk-icon="icon:close" aria-hidden="true" style="min-width:20px;"></span>
                    <span class="visually-hidden uk-hidden"><?php echo Text::_('JLIB_FORM_BUTTON_CLEAR'); ?></span>
                <?php } else { ?>
                    <span><?php echo Text::_('JLIB_FORM_BUTTON_CLEAR'); ?></span>
                <?php } ?>
            </button>
        <?php } ?>
    </div>
