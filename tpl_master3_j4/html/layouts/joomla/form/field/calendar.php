...
empty($class)     ? $attributes['class'] = 'uk-input form-control' : $attributes['class'] = 'uk-input form-control ' . $class;
...
<div class="field-calendar">
    <div class="uk-button-group">
        <input type="text" id="<?php echo $id; ?>" name="<?php echo $name; ?>" value="<?php echo htmlspecialchars(($value !== '0000-00-00 00:00:00') ? $value : '', ENT_COMPAT, 'UTF-8'); ?>" <?php echo !empty($description) ? ' aria-describedby="' . ($id ?: $name) . '-desc"' : ''; ?> <?php echo $attributes; ?> <?php echo $dataAttribute ?? ''; ?> <?php echo !empty($hint) ? 'placeholder="' . htmlspecialchars($hint, ENT_COMPAT, 'UTF-8') . '"' : ''; ?> data-alt-value="<?php echo htmlspecialchars($value, ENT_COMPAT, 'UTF-8'); ?>" autocomplete="off">
        <button type="button" class="<?php echo ($readonly || $disabled) ? 'hidden ' : ''; ?>btn btn-primary" id="<?php echo $id; ?>_btn" title="<?php echo Text::_('JLIB_HTML_BEHAVIOR_OPEN_CALENDAR'); ?>" <?php echo $calendarAttrsStr; ?>>
            <?php if ($jsIcons) { ?>
                <span data-uk-icon="icon:calendar" aria-hidden="true" style="min-width:20px;"></span>
                <span class="visually-hidden uk-hidden"><?php echo Text::_('JLIB_HTML_BEHAVIOR_OPEN_CALENDAR'); ?></span>
            <?php } else { ?>
                <span><?php echo Text::_('JLIB_HTML_BEHAVIOR_OPEN_CALENDAR'); ?></span>
            <?php } ?>
        </button>
    </div>
</div>
