...
$inputAttributes = array(
    'type' => 'text', 'id' => $id, 'class' => 'uk-input form-control field-user-input-name', 'value' => $this->escape($userName)
);
if ($class) {
...
if (!$readonly) {
    ...
    $modalHTML = HTMLHelper::_(
        ...
        array(
            ...
            'modalWidth'  => 80,
            'bodyHeight'  => 60,
            'footer'      => '<button type="button" class="uk-button uk-button-primary" data-bs-dismiss="modal" ' . $onclick . '>' . Text::_('JCANCEL') . '</button>',
        )
    );

    $app->getDocument()->getWebAssetManager()->useScript('webcomponent.field-user');
}
?>
...
<joomla-field-user class="field-user-wrapper" url="<?php echo (string) $uri; ?>" modal=".modal" modal-width="100%" modal-height="400px" input=".field-user-input" input-name=".field-user-input-name" button-select=".button-select">
    <div class="uk-button-group">
        ...
        <?php if (!$readonly) { ?>
            <button type="button" class="uk-button uk-button-primary button-select"
                title="<?php echo Text::_('JLIB_FORM_CHANGE_USER'); ?>" data-uk-toggle="target:#<?php echo 'userModal_' . $id; ?>">
                <?php if ($jsIcons) { ?>
                <span data-uk-icon="icon:user" aria-hidden="true" style="min-width:20px;"></span>
                <span class="visually-hidden uk-hidden"><?php echo Text::_('JLIB_FORM_CHANGE_USER'); ?></span>
                <?php } else { ?>
                <span><?php echo Text::_('JLIB_FORM_CHANGE_USER'); ?></span>
                <?php } ?>
            </button>
        <?php } ?>
    </div>
    ...
    ?>
    <?php if (!$readonly) { ?>
        <input type="hidden" id="<?php echo $id; ?>_id" name="<?php echo $name; ?>" value="<?php echo $this->escape($value); ?>" class="field-user-input <?php echo $class ? (string) $class : '' ?>" data-onchange="<?php echo $this->escape($onchange); ?>">
        <?php echo $modalHTML; ?>
    <?php } ?>
</joomla-field-user>
